...
router.post('/parties', (req, res, next) => {
  if (req.body.party_name && req.body.date && req.body.time && req.body.location && req.body.host && req.body.max_guests && req.body.guests) {
...
router.put('/parties/:id/join', (req, res, next) => {
  ...
  if (email) {
    Party.findByIdAndUpdate(req.params.id, { $push: { guests: email } }, { new: true })
      .then((data) => res.json(data))
      .catch(next);
  } else {
    res.json({
...
router.put('/parties/:id/leave', (req, res, next) => {
  const { email } = req.body;
  if (email) {
    Party.findByIdAndUpdate(req.params.id, { $pull: { guests: email } }, { new: true })
      .then((data) => res.json(data))
      .catch(next);
  } else {
    res.json({
