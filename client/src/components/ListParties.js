...
const ListParties = ({ isLoggedIn, parties, deleteParty, joinParty, leaveParty }) => {
    ...
    <ul>
      {partyList && partyList.length > 0 ? (
        partyList.map((party) => {
          ...
          const guestNames = party.guests.map((guest) => guest.split('@')[0]).join(', ');
          return (
            <li key={party._id}>
              ...
              <p>Guests: {party.guests.length > 0 ? guestNames : "No guests yet"}</p>
              { !isLoggedIn
                ? <p>Log in to join</p>
              ...
              {isLoggedIn && party.host === currentUserEmail && <button onClick={() => deleteParty(party._id)}>Delete</button>}
            </li>
          );
        })
      ) : (
